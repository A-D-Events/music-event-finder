...
public class EventParser {
	...
	public static List<Event> parseEvents(JSONObject json) {
		...
		if (!json.has("_embedded")) return eventsList;
		...
		for (int i = 0; i < events.length(); i++) {
			...
			// 🧼 FILTRO: descartar eventos incompletos
			if (name.equals("Unknown") || artist.equals("Unknown artist") || date.equals("N/A")) {
				continue;
			}

			// 🧼 FILTRO: evitar duplicados simples (por nombre + artista + fecha)
			boolean alreadyExists = eventsList.stream().anyMatch(ev ->
					ev.name.equals(name) &&
							ev.artist.equals(artist) &&
							ev.date.equals(date));
